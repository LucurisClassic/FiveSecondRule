...
do -- private scope
    ...
    function Refresh()
        ...
        -- FOREGROUND
        tickbar:SetStatusBarTexture("Interface\\TARGETINGFRAME\\UI-StatusBar", "OVERLAY")
        ...
        if (not tickbar.bg) then
            tickbar.bg = tickbar:CreateTexture(nil, "OVERLAY")
        end
        ...
        if (FiveSecondRule_Options.showSpark) then
            if (not tickbar.bg.spark) then
                local spark = tickbar:CreateTexture(nil, "HIGHLIGHT")
                spark:SetTexture("Interface\\Addons\\UI-CastingBar-Spark")
                spark:SetWidth(16)        
